...
import check_distribution  # noqa: E402 -- exercise standalone tools after adding their import root
import check_mcp  # noqa: E402 -- exercise standalone tools after adding their import root
import smoke_onboarding  # noqa: E402 -- exercise standalone tools after adding their import root
import smoke_plugin  # noqa: E402 -- exercise standalone tools after adding their import root
...
class DiscoveryTests(unittest.TestCase):
    ...
    def test_sdk_pin_is_read_from_plugin_not_duplicated(self):
        ...
        with patch("importlib.metadata.version", return_value="9.8.6"):
...
class GateTests(unittest.TestCase):
    ...
    def test_mcp_failure_stops_the_gate_after_a_passing_base(self):
        ...
        with patch("check_distribution.subprocess.run", side_effect=outcomes) as run:
            with self.assertRaisesRegex(check_distribution.CheckFailure, "mcp failed"):
    ...
    def test_unavailable_process_leaves_a_failed_stage(self):
        report = {"checks": []}
        with patch("check_distribution.subprocess.run", side_effect=FileNotFoundError):
    ...
    def test_unsafe_archive_does_not_write_outside_its_new_directory(self):
        for name in (
            ...
            with self.subTest(name=name), tempfile.TemporaryDirectory() as directory:
                ...
                archive = root / "plugin.zip"
                with zipfile.ZipFile(archive, "w") as output:
                    output.writestr(name, "unexpected")
