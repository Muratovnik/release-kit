...
def check_secret_detection(root: Path, environment: dict[str, str]) -> None:
    """A never-issued token exercises the real default rule, not a custom allowlist."""
    path = root / "synthetic-credential.txt"
    # Construct at runtime so source/history does not contain a full token fixture.
    # This value is not issued by GitHub and is never sent to a credential API.
...
def smoke(root: Path, artifact: Path) -> None:
    ...
    check_secret_detection(root, environment)
