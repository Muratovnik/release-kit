...
This repository's own `CHANGELOG.md` uses the `conventional-changelog` profile, with
one entry per published release drafted by `relkit notes <version> --draft` from the
commits of that release's range. Every bullet is one commit, named by its scope and
...
and put the reasoning in the commit body. Do not hand-write a bullet and do not restate
a commit; draft the entry, then review and commit it. Context a subject cannot carry
goes in a `Highlights` section, which the profile exempts from the commit-link rule.
Adopters choosing `legacy` or `strict` keep curated prose, and for them the guidance
stays: keep notes user-facing, preserve historical entries and move detail to linked
references.
